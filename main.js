...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderHeritageList(filterCategory = '') {
        ...
        filteredData.forEach(item => {
            ...
            if (item.modelPath) {
                ...
                mediaHtml = `<img src="${item.imagePath}" style="width:100%; height:100%; object-fit:cover;">`;
            } else {
    ...
    function openDetail(item) {
        ...
        if (item.modelPath) {
            heroArea.innerHTML = `<model-viewer src="${item.modelPath}" auto-rotate camera-controls style="width:100%; height:100%;"></model-viewer>`;
        } else if (item.imagePath) {
